class ContainerK8sSnapshot < Formula
  ...
  url "https://github.com/stephenlclarke/container-k8s/releases/download/homebrew-snapshot-bootstrap/container-k8s-plugin-snapshot-bootstrap-debug-arm64.tar.gz"
  sha256 :no_check
  ...
  conflicts_with "container-k8s", because: "both install the container-k8s command and k8s plugin"
  ...
  def caveats
    <<~EOS
      ...
      This formula installs the snapshot bootstrap debug prebuilt release asset:
        container-k8s-plugin-snapshot-bootstrap-debug-arm64.tar.gz
    EOS
  end
